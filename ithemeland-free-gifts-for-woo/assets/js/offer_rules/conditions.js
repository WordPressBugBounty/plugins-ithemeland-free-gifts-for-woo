...
jQuery(document).ready(function ($) {
    $(document).on('click', '.wgb-add-condition', function () {
        let rule = $(this).closest('.wgb-offer-rule-item');
        let ruleID = rule.attr('data-id');
        let conditionID = parseInt($(this).closest('.wgb-offer-rule-item').find('.wgb-condition-items .wgb-offer-rule-item-sortable-item').length);

        if (WGBL_OFFER_RULES_DATA.condition.row) {
            rule.find('.wgb-condition-items').append((WGBL_OFFER_RULES_DATA.condition.row).replaceAll('set_condition_id_here', conditionID).replaceAll('set_rule_id_here', ruleID)).ready(function () {
                let newItem = rule.find('.wgb-condition-items .wgb-offer-rule-item-sortable-item').last();
    ...
    $(document).on('change', '.wgb-offer-rule-condition-item', function () {
        let ruleID = $(this).closest('.wgb-offer-rule-item').attr('data-id');
        let conditionID = $(this).closest('.wgb-offer-rule-item-sortable-item').attr('data-id');
        let item = $(this).closest('.wgb-offer-rule-item-sortable-item').find('.wgb-condition-extra-fields');
        item.attr('data-id', conditionID);

        if (WGBL_OFFER_RULES_DATA.condition.extra_fields[$(this).val()]) {
            item.html(WGBL_OFFER_RULES_DATA.condition.extra_fields[$(this).val()].replaceAll('set_condition_id_here', conditionID).replaceAll('set_rule_id_here', ruleID)).ready(function () {
                wgbResetData();
            });
        }
    });
    ...
    $(document).on('click', '.wgb-condition-delete', function () {
        ...
        $(this).closest('.wgb-offer-rule-item-sortable-item').remove();
