...
jQuery(document).ready(function ($) {
  $(document).on("click", ".wgbl-add-condition", function () {
    ...
    if (WGBL_RULES_DATA.condition.row) {
      rule
        .find(".wgbl-condition-items")
        .append(WGBL_RULES_DATA.condition.row.replaceAll("set_condition_id_here", conditionID).replaceAll("set_rule_id_here", ruleID))
        .ready(function () {
  ...
  $(document).on("change", ".wgbl-condition-user-meta-field-type", function () {
    ...
    let conditionID = $(this).closest(".wgbl-rule-item-sortable-item").attr("data-id");
    $(this)
      .closest(".wgbl-condition-extra-fields")
      .find(".wgbl-condition-extra-fields-col-4")
      .html(wgblGetConditionUserMetaFieldTypeFields($(this).val(), ruleID, conditionID));
  });
  ...
  $(document).on("change", ".wgbl-condition-coupons-applied-type", function () {
    ...
    $(this)
      .closest(".wgbl-condition-extra-fields")
      .find(".wgbl-condition-extra-fields-col-4")
      .html(wgblGetConditionCouponsAppliedTypeFields($(this).val(), ruleID, conditionID));
    wgblResetData();
  });
});
  ...
  switch (type) {
    ...
    case "none":
      output =
        ...
        "][condition][" +
        conditionID +
        '][coupons][]" class="wgbl-select2-coupons wgbl-select2-option-values" data-option-name="coupons" data-type="select2" data-placeholder="Select ..." required multiple></select>';
      break;
    default:
      output = "";
  }
