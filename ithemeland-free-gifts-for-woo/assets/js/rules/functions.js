...
function wgblRuleDuplicate(ruleItem) {
  ...
  ruleItem
    ...
    .ready(function () {
      ...
      duplicated
        .find('input[name="rule[' + dID + '][uid]"]')
        .val(newUid)
        .change();
      duplicated.find(".wgbl-rule-method-id").text("ID: " + newUid);
      duplicated
        .find('select[name="rule[' + dID + '][quantities_based_on]"]')
        .val(ruleItem.find('select[name="rule[' + sID + '][quantities_based_on]"]').val())
        .change();
...
function wgblDuplicateProductBuyItemFields(sourceId, destinationId, iteration) {
  ...
  wgblDuplicateMultipleSelect2Item(
    ...
    success: function (response) {
      ...
      if (response.has_update == "yes") {
        if (response.license_is_valid == "yes") {
          jQuery(".wgbl-check-for-update").html(
            '<a href="' +
              response.update_link +
              '" class="wgbl-button-blue wgbl-button-blue wgbl-update-button">Update Now</a><strong class="wgbl-new-version">New version: ' +
