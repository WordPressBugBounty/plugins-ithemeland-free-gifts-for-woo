jQuery(document).ready(function ($) {
  ...
  $(document).on("change", ".wgbl-rule-item-status", function () {
    $(this)
      .closest(".wgbl-rule-item")
      .removeClass("wgbl-rule-enable")
      .removeClass("wgbl-rule-disable")
      .addClass("wgbl-rule-" + $(this).val());
  });
  ...
  $(document).on("change", ".wgbl-rule-method", function () {
    ...
    if ($(this).val() == "free_shipping") {
      item.find('div[data-type="quantities"] .wgbl-rule-section h3').show(); // Show the element
    } else {
      item.find('div[data-type="quantities"] .wgbl-rule-section h3').hide(); // Hide the element
    }
    ...
    switch ($(this).val()) {
      case "simple":
        if ($.isFunction(window.wgblSimpleMethod)) {
          wgblSimpleMethod(id);
        }
        break;
      case "tiered_quantity":
        if ($.isFunction(window.wgblTieredQuantityMethod)) {
          wgblTieredQuantityMethod(id);
        }
        break;
      case "bulk_quantity":
        if ($.isFunction(window.wgblBulkQuantityMethod)) {
          wgblBulkQuantityMethod(id);
        }
        break;
      case "bulk_pricing":
        if ($.isFunction(window.wgblBulkPricingMethod)) {
          wgblBulkPricingMethod(id);
        }
        break;
      case "subtotal":
        if ($.isFunction(window.wgblSubTotalMethod)) {
          wgblSubTotalMethod(id);
        }
        break;
      case "subtotal_repeat":
        ...
        break;
      case "buy_x_get_x":
        if ($.isFunction(window.wgblBuyXGetXMethod)) {
          wgblBuyXGetXMethod(id);
        }
        break;
      case "buy_x_get_x_repeat":
        ...
        break;
      case "buy_x_get_y":
        if ($.isFunction(window.wgblBuyXGetYMethod)) {
          wgblBuyXGetYMethod(id);
        }
        break;
      ...
      case "cheapest_item_in_cart":
        if ($.isFunction(window.wgblCheapestItemInCart)) {
          wgblCheapestItemInCart(id);
        }
        break;
      case "free_shipping":
        if ($.isFunction(window.wgblFreeShippingMethod)) {
          wgblFreeShippingMethod(id);
        }
        break;
      default:
        return false;
      ...
      default:
        ...
        setTimeout(function () {
          if ($("#wgbl-settings-layout option:selected").attr("data-type") !== $this.val()) {
            $("#wgbl-settings-layout")
              .val(
                $('#wgbl-settings-layout option[data-type="' + $this.val() + '"]')
                  .first()
                  .attr("value")
              )
              .trigger("change");
  ...
  $(document).on("click", "#wgbl-rules-save-changes", function () {
    ...
    jQuery("#wgbl-rules input:required")
      .filter(function () {
    ...
    let select2Items = [];
    jQuery("#wgbl-rules select:required")
      .filter(function () {
